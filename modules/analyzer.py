...
class MarketAnalyzer:
    ...
    def generate_report(self, market_data, news_data, institutional_data=None, prev_report_path=None, commodity_data=None, macro_events=None, tech_catalyst_events=None, volume_data=None, deep_dive_data=None, signal_summary=None):
        """
...
## 本地量化訊號包（由程式計算，非 AI 推論）
{signal_summary if signal_summary else '（尚未建立本地量化訊號包）'}

使用規則：
- 「主力操作假說」只能寫成假說，不能寫成已確認事實。
- 報告必須明確說出隔日主力操作方向的驗收條件：續量/量縮、守住哪條均線、是否跌破今日低點、法人是否轉買/轉賣。
- 如果本地訊號包的信心分數低於 50/100，請降低語氣，寫「觀察」而不是「預測」。
...
================================================================
這些是我們在過去追蹤到的市場敘事、大事件、法人買賣與成交量結構。
...
================================================================
優先使用「本地量化訊號包」標示的最強異常。除非原始數據明顯反駁，否則不要自行改選主題。
...
6. 禁用空泛套話：正文不得出現「資金輪動」四字；「資金脈絡」不得作為段落標題。請改為抓出「三大法人與成交量之間的主力買賣邏輯」，每次談買賣方向，都必須具體寫出「誰買、誰賣、金額/成交量、哪個榜單支持、明天驗收條件」。
...
================================================================
優先從「本地量化訊號包」的明日觀察候選中挑選；若候選為空，原則上輸出「今日數據無安全明牌，現金為王，靜待訊號。」
...
（用具體榜單與數字說明：外資買超/賣超、成交量排名、技術位、明日驗收條件。必須包含一段「主力操作假說」：隔日偏多/偏空/中性、確認條件、反證條件。不要寫抽象口號。）
